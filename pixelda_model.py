...
def create_model(hparams,
                 target_images, target_images_w_labels,
                 source_images=None,
                 is_training=False,
                 noise=None,
                 num_classes=None):
  ...
  with slim.arg_scope(
    ...
    with tf.variable_scope('discriminator'):
      with tf.variable_scope('task_tower'):
        end_points['source_lateral_task_logits'], end_points['source_head_task_logits'], nets = pixelda_task_towers.add_task_specific_model(
          source_images,
          hparams,
          ...
          reuse_shared=False)
        end_points['transferred_lateral_task_logits'], end_points['transferred_head_task_logits'], _ = pixelda_task_towers.add_task_specific_model(
          end_points['transferred_images'],
          hparams,
          num_classes = num_classes,
          is_training=is_training,
          ...
          reuse_shared=True)
        end_points['target_lateral_task_logits'], end_points['target_head_task_logits'], _ = pixelda_task_towers.add_task_specific_model(
          target_images,
          hparams,
          num_classes = num_classes,
          is_training=is_training,
          ...
          private_scope='transferred_task_classifier',
          reuse_shared=True)
        end_points['target_lateral_task_logits_w_labels'], end_points['target_head_task_logits_w_labels'], _ = pixelda_task_towers.add_task_specific_model(
          target_images_w_labels,
          hparams,
          num_classes = num_classes,
          is_training=is_training,
          reuse_private=True,
...
def predict_domain(images,
  ...
  with tf.variable_scope(scope, 'discriminator', [images], reuse=reuse):
    with tf.variable_scope('domain_tower',reuse=reuse):
      ...
      with slim.arg_scope(
        ...
        while net.shape.as_list()[1] > hparams.projection_shape_size and net.shape.as_list()[2] > hparams.projection_shape_size:
