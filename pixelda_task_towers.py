  ...
  with slim.arg_scope(
    ...
    with slim.arg_scope([slim.conv2d], padding='SAME'):
      if model == 'doubling_pose_estimator':
        ...
        lateral_logit, head_logit, _ = resnet_classifier(images, **kwargs)
      else:
        raise ValueError('Unknown task classifier %s' % model)

  return lateral_logit, head_logit, quaternion_pred
...
def resnet_classifier(images,
  ...
  #with tf.variable_scope(shared_scope, reuse=reuse_shared):
    net['conv1'] = slim.conv2d(images, filters[0], [kernels[0],kernels[0]], strides[0],padding='SAME',normalizer_fn=slim.batch_norm,activation_fn=tf.nn.relu)
    net['pool1'] = tf.nn.max_pool(net['conv1'],[1,3,3,1],[1,2,2,1],'SAME')

  with tf.variable_scope(shared_scope, reuse=reuse_shared):
    net['conv2_1'] = _residual_block(net['pool1'])
    net['conv2_2'] = _residual_block(net['conv2_1'])
    net['conv3_1'] = _residual_block_first(net['conv2_2'], filters[2], strides[2])
    net['conv3_2'] = _residual_block(net['conv3_1'])
    net['conv4_1'] = _residual_block_first(net['conv3_2'], filters[3], strides[3])
    net['conv4_2'] = _residual_block(net['conv4_1'])
    net['conv5_1'] = _residual_block_first(net['conv4_1'], filters[4], strides[4])
    net['conv5_2'] = _residual_block(net['conv5_1'])

    net['average_pooling'] = tf.reduce_mean(net['conv5_2'],[1,2])
    head_logit    = slim.fully_connected(net['average_pooling'],num_classes,scope='head')
    lateral_logit = slim.fully_connected(net['average_pooling'],num_classes,scope='lateral')
    
    is_trainable = lambda x: x in tf.trainable_variables()
    var_list = filter(is_trainable, slim.get_model_variables(private_scope))
 
    return lateral_logit, head_logit, net

def _residual_block(x):
  num_channel = x.get_shape().as_list()[-1]
  shortcut = x
  x = slim.conv2d(x, num_channel, [3,3], 1, padding='SAME', normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu)
  x = slim.conv2d(x, num_channel, [3,3], 1, padding='SAME', normalizer_fn=slim.batch_norm, activation_fn=None)
  x = x + shortcut
  x = tf.nn.relu(x)
  return x

def _residual_block_first(x, out_channel, strides):
  ...
  if in_channel == out_channel:
    ...
    shortcut = slim.conv2d(x, out_channel, [1,1], strides, padding='SAME')
  # Residual
  x = slim.conv2d(x, out_channel, [3,3], strides,padding='SAME',normalizer_fn=slim.batch_norm, activation_fn = tf.nn.relu)
  x = slim.conv2d(x, out_channel, [3,3], 1, padding='SAME', normalizer_fn = slim.batch_norm, activation_fn = None)
  x = x + shortcut
  x = tf.nn.relu(x)
  return x
