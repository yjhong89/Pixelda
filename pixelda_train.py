...
flags.DEFINE_string('train_log_dir', '/home/intel/yjhong89/domaina/input_mask/contrast_no_content_entropy_gstep',
...
flags.DEFINE_integer('save_interval_secs', 40,
...
flags.DEFINE_string('source_dataset', 'source', 'The name of the source dataset.'
...
flags.DEFINE_string('source_dataset_dir', 'data_source',
                    'The directory where the source datasets can be found.')
flags.DEFINE_string('target_dataset_dir', 'data_target',
  ...
  with tf.train.MonitoredSession(
    ...
    while not session.should_stop():
      ...
      for _ in range(hparams.discriminator_steps):
        ...
        _ = session.run(alpha_train_op)
        if np_global_step % FLAGS.print_loss_steps == 0:
          tf.logging.info('Step %d: Discriminator Loss = %.10f', np_global_step,
                          loss)
...
def run_training(run_dir, checkpoint_dir, hparams):
  ...
  f = open(os.path.join(run_dir,'params.txt'),'w')
  f.write('Input mask: '+str(hparams.input_mask))
  f.write('\nnoisedim :' +str(hparams.noise_dims)) 
  f.write('\ndomain_loss_weight : '+str(hparams.domain_loss_weight))
  f.write('\nstyle_transfer_loss_weight : '+str(hparams.style_transfer_loss_weight))
  f.write('\nsource_head and lateral task loss weight : '+str(hparams.source_head_task_loss_weight)+' '+str(hparams.source_lateral_task_loss_weight))
  f.write('\ntransfer head and lateral task loss weight : '+str(hparams.transferred_head_task_loss_weight)+' '+str(hparams.transferred_lateral_task_loss_weight))
  f.write('\ntarget head and alteral task loss weight : '+str(hparams.target_head_task_loss_weight)+' '+str(hparams.target_lateral_task_loss_weight))
  f.write('\ntask loss in g weight : '+str(hparams.task_loss_in_g_weight)+',' +str(hparams.task_tower_in_g_step))
  f.write('\ntransferred content loss weight : '+str(hparams.transferred_similarity_loss_weight))
  f.write('\nentropy : '+str(hparams.task_classifier_entropy_weight))
  f.write('\ncontrast :'+str(hparams.task_contrast_penalty))
  f.close()
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(tf.train.replica_device_setter(ps_tasks=FLAGS.ps_tasks)):
      ...
      target_images, _ = dataset_factory.provide_batch(
        ...
        'train', FLAGS.target_dataset_dir, FLAGS.num_readers,
        hparams.batch_size, FLAGS.num_preprocessing_threads) 

      target_images_w_labels, target_labels = dataset_factory.provide_batch(
        FLAGS.target_dataset,
        'label_train', FLAGS.target_dataset_dir, FLAGS.num_readers,
        hparams.batch_size, FLAGS.num_preprocessing_threads)
      ...
      if hparams.arch not in ['dcgan']:
        source_images, source_labels = dataset_factory.provide_batch(
            FLAGS.source_dataset, 'train', FLAGS.source_dataset_dir, FLAGS.num_readers,
            hparams.batch_size, FLAGS.num_preprocessing_threads)
        ...
        # left / center / right w.r.t the lane
        source_lateral_labels = (source_labels['class'] % 9) / 3
        # left / straight / right head w.r.t the lane
      ...
      end_points = pixelda_model.create_model(
          hparams, target_images, target_images_w_labels,
...
#      ]
#      session_creator = tf.train.ChiefSessionCreator(scaffold=scaffold, checkpoint_dir='/home/intel/yjhong89/domaina/ckpt', master='')
#      hooks.extend(chief_only_hooks)
#      hooks.append(tf.train.StepCounterHook(output_dir='/home/intel/yjhong89/domaina/output'))
#      with tf.train.MonitoredSession(session_creator=session_creator, hooks=hooks) as sess:
#        print('===========================================')
#        x = sess.run(end_points['transferred_images'])
#        print(x.shape)
#        sys.exit()
      #################################
      ...
      generator_loss = pixelda_losses.g_step_loss(
          ...
          hparams,
          num_classes = num_target_classes)
      discriminator_loss, alpha_train_op = pixelda_losses.d_step_loss(
          end_points, source_lateral_labels, source_head_labels, target_lateral_labels, target_head_labels, num_target_classes, hparams)
      ...
      if 'source_images_recon' in end_points:
        pixelda_utils.summarize_transferred_grid(
            ...
            source_images, name='Source_Reconstruction')
      ...
      pixelda_utils.summaries_color_distributions(
          tf.abs(source_images - end_points['transferred_images']),'Abs(Source_minus_Transferred')
